...
public class EfficientDocument extends Document {
	...
	private void processText()
	{
		...
		for (int k=0; k < tokens.size();k++) {
			if (isWord(tokens.get(k)) && k+1 != tokens.size()) {
				this.numWords ++;
				int count = countSyllables(tokens.get(k));
				this.numSyllables = this.numSyllables + count;
			}
			//else if ((isWord(tokens.get(k)) && k-1 == tokens.size()) || (!isWord(tokens.get(k)) && k != 0) || (!isWord(tokens.get(k)) && tokens.size()==1)) {
			else if (!isWord(tokens.get(k))&& k !=0) {
				this.numSentences ++;
			}
			else if (isWord(tokens.get(k)) && k+1 == tokens.size()) {
				this.numWords ++;
				this.numSentences ++;
				int count = countSyllables(tokens.get(k));
				this.numSyllables = this.numSyllables + count;
