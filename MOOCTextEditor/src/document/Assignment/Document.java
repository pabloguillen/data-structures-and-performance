...
public abstract class Document {
	...
	protected int countSyllables(String word)
	{
		...
		for (int i=0; i < word.length();i++) {
			...
			if (i>0) {
				char prevletter = letters [i-1];
			    if (isVowel(currletter) && !isVowel(prevletter)) {
			    	vowelcount++;
			}
			}
			else if (isVowel(currletter)) {
				vowelcount++;
			}	
		}
		char lastletter = word.charAt(word.length()-1);
		if (word.length()>=2) {
			char prevletter = word.charAt(word.length()-2);
		if (lastletter == 'e' && vowelcount >1 && !isVowel(prevletter)) {
